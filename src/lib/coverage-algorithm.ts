...
interface CoverageResult {
  ...
  assignedRole: RolePriority | null;
  reason: string;
 ...
 */
export async function assignCoverageForAbsence(
  prisma: PrismaClient, 
  absence: Absence & { department?: string }
  ...
  function isAvailable(staffId: string, period: string): boolean {
    ...
    const covering = coverageAssignments.find(ca => 
      (ca as any)[`period${period}`] === staffId
    );
  ...
  function getPeriodMatch(neededPeriods: string[], availablePeriods: string[]): {
    canCover: string[];
    cannotCover: string[];
    ...
    let matchType: 'perfect' | 'partial' | 'none';
  ...
  function getCurrentLoad(staffId: string): number {
    let load = 0;
  ...
  function hasQualificationMatch(candidate: CandidateStaff, subject: string): boolean {
  ...
  async function checkSpecialConstraints(candidate: CandidateStaff, period: string): Promise<boolean> {
    // Check load limits
    ...
    if (consecutivePeriods > 4) return false; // Max 4 consecutive periods
    ...
    if (candidate.role === 'external_substitute') {
      ...
      if (currentDailyLoad >= 6) return false; // Substitutes limited to 6 periods per day
    }
  ...
  async function getDailyLoadForSubstitute(substituteId: string, date: Date): Promise<number> {
    const dateStr = new Date(date).toISOString().split('T')[0];
    ...
    for (const assignment of existingAssignments) {
      for (const period of ['1st', '2nd', '3rd', '4th', '5th', '6th', '7th', '8th']) {
        if ((assignment as any)[`period${period}`] === substituteId) {
          dailyLoad++;
        }
      }
    }
  ...
  async function getConsecutivePeriods(staffId: string, dayOfWeek: string, newPeriod: string): Promise<number> {
    ...
    for (let i = newPeriodIndex - 1; i >= 0; i--) {
      ...
      const isBusy = masterSchedule.some(ms => 
        ms.teacherId === staffId && ms.period === period && ms.isTeaching
      ) || coverageAssignments.some(ca => 
        (ca as any)[`period${period}`] === staffId
      );
      
      if (isBusy) consecutive++;
      else break;
    }
  ...
  let allAssignments: { [period: string]: { candidate: CandidateStaff; role: RolePriority; reason: string } } = {};
  let uncoveredPeriods = [...periods]; // Start with all periods uncovered
  let candidatesEvaluated = 0;

  // Iterate through role priorities
  for (const role of ROLE_PRIORITY) {
    if (uncoveredPeriods.length === 0) break; // All periods covered

    let candidates: CandidateStaff[] = [];
        ...
        candidates = await Promise.all(allSubs.map(async (sub) => {
          ...
          return {
            ...
            subjectSpecialties: Array.isArray(sub.subjectSpecialties) 
              ...
              : JSON.parse(sub.subjectSpecialties as string),
            availability,
            currentLoad,
            maxLoad,
