...
interface DepartmentConfig {
  ...
  name: string;
  code: string;
  sameDepartmentCoverage: boolean;
  crossDepartmentCoverage: boolean;
...
interface LoadLimitConfig {
  ...
  minPrepPeriodsPerDay: number;
  minLunchPeriodsPerDay: number;
  constraints: any;
  isActive: boolean;
}
...
export default function ConfigurationPage() {
  ...
  const [loadLimitConfigs, setLoadLimitConfigs] = useState<LoadLimitConfig[]>([]);
  const [constraintConfigs, setConstraintConfigs] = useState<ConstraintConfig[]>([]);
  ...
  const [editingLoadLimit, setEditingLoadLimit] = useState<LoadLimitConfig | null>(null);
  const [editingConstraint, setEditingConstraint] = useState<ConstraintConfig | null>(null);
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="schedule" className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <Button onClick={() => setEditingSchedule({} as ScheduleConfig)}>
              Add Schedule Type
            </Button>
          </div>
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => setEditingSchedule(config)}
                      >
                        Edit
                      </Button>
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => toggleActive('schedule-configs', config.id, !config.isActive)}
                      >
          ...
          {editingSchedule && (
            <ScheduleTypeForm
              ...
              onSave={(data: any) => saveConfiguration('schedule-configs', data)}
        ...
        <TabsContent value="periods" className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <Button onClick={() => setEditingPeriod({} as PeriodConfig)}>
              Add Period
            </Button>
          </div>
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => setEditingPeriod(config)}
                      >
                        Edit
                      </Button>
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => toggleActive('period-configs', config.id, !config.isActive)}
                      >
                        {config.isActive ? 'Deactivate' : 'Activate'}
                      </Button>
                    </div>
          ...
          {editingPeriod && (
            <PeriodForm
              config={editingPeriod}
              onSave={(data: any) => saveConfiguration('period-configs', data)}
        ...
        <TabsContent value="departments" className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <Button onClick={() => setEditingDepartment({} as DepartmentConfig)}>
              Add Department
            </Button>
          </div>
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => setEditingDepartment(config)}
                      >
                        Edit
                      </Button>
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => toggleActive('department-configs', config.id, !config.isActive)}
                      >
                        {config.isActive ? 'Deactivate' : 'Activate'}
                      </Button>
                    </div>
          ...
          {editingDepartment && (
            <DepartmentForm
              config={editingDepartment}
              onSave={(data: any) => saveConfiguration('department-configs', data)}
        ...
        <TabsContent value="load-limits" className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <Button onClick={() => setEditingLoadLimit({} as LoadLimitConfig)}>
              Add Load Limit
            </Button>
          </div>

          <div className="grid gap-4">
            {loadLimitConfigs.map((config) => (
              <Card key={config.id}>
                <CardHeader>
                  <div className="flex justify-between items-start">
                    <div>
                    ...
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => setEditingLoadLimit(config)}
                      >
                        Edit
                      </Button>
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => toggleActive('load-limit-configs', config.id, !config.isActive)}
                      >
                        {config.isActive ? 'Deactivate' : 'Activate'}
                      </Button>
                    </div>
          ...
          {editingLoadLimit && (
            <LoadLimitForm
              config={editingLoadLimit}
              onSave={(data: any) => saveConfiguration('load-limit-configs', data)}
              onCancel={() => setEditingLoadLimit(null)}
              saving={saving}
            />
          )}
        </TabsContent>
        ...
        <TabsContent value="constraints" className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <Button onClick={() => setEditingConstraint({} as ConstraintConfig)}>
              Add Constraint
            </Button>
          </div>
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => setEditingConstraint(config)}
                      >
                        Edit
                      </Button>
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => toggleActive('constraint-configs', config.id, !config.isActive)}
                      >
                        {config.isActive ? 'Deactivate' : 'Activate'}
                      </Button>
                    </div>
          ...
          {editingConstraint && (
            <ConstraintForm
              config={editingConstraint}
              onSave={(data: any) => saveConfiguration('constraint-configs', data)}
...
// Form Components
function ScheduleTypeForm({ config, onSave, onCancel, saving }: any) {
...
function PeriodForm({ config, onSave, onCancel, saving }: any) {
  const [formData, setFormData] = useState(config);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-4">
            <div className="flex items-center space-x-2">
              <Switch
                id="isTeaching"
                checked={formData.isTeaching || false}
            ...
            <div className="flex items-center space-x-2">
              <Switch
                id="isCoverable"
                checked={formData.isCoverable || false}
                onCheckedChange={(checked) => setFormData({ ...formData, isCoverable: checked })}
              />
...
function DepartmentForm({ config, onSave, onCancel, saving }: any) {
  const [formData, setFormData] = useState(config);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <Input
              ...
              min="1"
              max="10"
              value={formData.coveragePriority || 1}
              onChange={(e) => setFormData({ ...formData, coveragePriority: parseInt(e.target.value) })}
            />
          </div>
          <div className="space-y-2">
            <div className="flex items-center space-x-2">
              <Switch
                id="sameDepartmentCoverage"
                checked={formData.sameDepartmentCoverage || false}
                onCheckedChange={(checked) => setFormData({ ...formData, sameDepartmentCoverage: checked })}
              />
              <Label htmlFor="sameDepartmentCoverage">Same Department Coverage</Label>
            </div>
            <div className="flex items-center space-x-2">
              <Switch
                id="crossDepartmentCoverage"
                checked={formData.crossDepartmentCoverage || false}
                onCheckedChange={(checked) => setFormData({ ...formData, crossDepartmentCoverage: checked })}
              />
              <Label htmlFor="crossDepartmentCoverage">Cross Department Coverage</Label>
            </div>
            <div className="flex items-center space-x-2">
              <Switch
                id="substituteCoverage"
                checked={formData.substituteCoverage || false}
                onCheckedChange={(checked) => setFormData({ ...formData, substituteCoverage: checked })}
              />
...
function LoadLimitForm({ config, onSave, onCancel, saving }: any) {
  const [formData, setFormData] = useState(config);
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input
                id="maxPeriodsPerDay"
                type="number"
                value={formData.maxPeriodsPerDay || 6}
                onChange={(e) => setFormData({ ...formData, maxPeriodsPerDay: parseInt(e.target.value) })}
              />
            </div>
            <div>
              ...
              <Input
                ...
                type="number"
                value={formData.maxPeriodsPerWeek || 30}
                onChange={(e) => setFormData({ ...formData, maxPeriodsPerWeek: parseInt(e.target.value) })}
              />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input
                ...
                type="number"
                value={formData.maxConsecutivePeriods || 4}
                onChange={(e) => setFormData({ ...formData, maxConsecutivePeriods: parseInt(e.target.value) })}
              />
            </div>
            <div>
              <Label htmlFor="minPrepPeriodsPerDay">Min Prep Periods/Day</Label>
              <Input
                id="minPrepPeriodsPerDay"
                type="number"
                value={formData.minPrepPeriodsPerDay || 1}
                onChange={(e) => setFormData({ ...formData, minPrepPeriodsPerDay: parseInt(e.target.value) })}
              />
            </div>
          </div>
          <div className="flex gap-2">
...
function ConstraintForm({ config, onSave, onCancel, saving }: any) {
  const [formData, setFormData] = useState(config);
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center space-x-2">
            <Switch
              id="isEnforced"
              checked={formData.isEnforced || false}
              onCheckedChange={(checked) => setFormData({ ...formData, isEnforced: checked })}
            />
