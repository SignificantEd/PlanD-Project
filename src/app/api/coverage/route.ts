...
export async function GET(request: NextRequest) {
  try {
    ...
    const assignment = await prisma.coverageAssignment.findFirst({
      where: { absenceId: absenceId as string },
    });
    ...
    // Resolve substitute names for assignments
    const assignments = await Promise.all(
      periods
        .filter(p => (assignment as any)[p.key])
        .map(async (p) => {
          const assignedValue = (assignment as any)[p.key];
          const assignmentType = (assignment as any)[p.typeKey] || 'Substitute';
          let assignedName = assignedValue;
          
          // If it's a substitute assignment, resolve the substitute name
          if ((assignmentType === 'Substitute' || assignmentType === 'External Sub') && assignedValue) {
            try {
              const substitute = await prisma.substitute.findUnique({
                where: { id: assignedValue }
              });
              if (substitute) {
                assignedName = substitute.name;
              }
            } catch (error) {
              console.error('Error resolving substitute name:', error);
            }
          }
    ...
    return NextResponse.json(
      { error: 'Failed to fetch coverage assignments' },
      { status: 500 }
    );
  }
}
  ...
  try {
    ...
    const coverageAssignment = await prisma.coverageAssignment.create({
      ...
      include: {
        ...
        // assignedTeacher: true, // Remove this if not in model
      },
    });
  ...
  try {
    ...
    const updated = await prisma.coverageAssignment.update({
      ...
      include: {
        ...
        // assignedTeacher: true, // Removed because it does not exist in the model
      },
    });
