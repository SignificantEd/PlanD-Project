...
export async function GET(request: NextRequest) {
  try {
    ...
    // If absenceId and substituteId are provided, fetch existing attendance
    if (absenceId && substituteId) {
      const attendance = await prisma.substituteAttendance.findUnique({
        where: { substituteId_absenceId: { substituteId, absenceId } },
      });
      return NextResponse.json(attendance);
    }

    // Original logic for fetching available substitutes
    if (!day || !period || !subject) {
      return NextResponse.json([]);
    }

    const allSubs = await prisma.substitute.findMany();
    const availableSubs = allSubs.filter((sub: any) => {
      const specialties = Array.isArray(sub.subjectSpecialties) ? sub.subjectSpecialties : JSON.parse(sub.subjectSpecialties as any);
      const availability = typeof sub.availability === 'object' ? sub.availability : JSON.parse(sub.availability as any);
      return (
        (!strict || specialties.includes(subject)) &&
        availability[day] &&
...
export async function POST(req: NextRequest) {
  ...
  const allPeriods: string[] = Array.isArray(absence.coverageAssignments)
    ? absence.coverageAssignments.map((ca: any) => ca.period)
    : [];

  // Determine periods covered by the sub
  let coveredPeriods: string[] = [];
  if (attendanceType === 'full') {
    ...
    coveredPeriods = allPeriods.filter((p: string) => ['1st','2nd','3rd','4th'].includes(p));
  } else if (attendanceType === 'half_pm') {
    coveredPeriods = allPeriods.filter((p: string) => ['5th','6th','7th','8th'].includes(p));
  } else if (attendanceType === 'custom' && Array.isArray(periodsWorked)) {
    coveredPeriods = periodsWorked;
  }
  const uncoveredPeriods = allPeriods.filter((p: string) => !coveredPeriods.includes(p));
