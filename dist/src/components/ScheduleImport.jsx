...
export default function ScheduleImport() {
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            const promises = importedData.map(row => fetch('/api/schedule', {
                method: 'POST',
                headers: {
    ...
    return (<div className="bg-white rounded-lg shadow-lg p-6">
      ...
      <div className="mb-4">
        ...
        <input type="file" accept=".csv" onChange={handleFileUpload} className="w-full border border-gray-300 rounded px-3 py-2"/>
      </div>
      ...
      {importedData.length > 0 && (<div className="mb-4">
          ...
          <div className="max-h-40 overflow-y-auto border border-gray-300 rounded">
            <table className="w-full text-sm">
              ...
              <tbody>
                {importedData.slice(0, 10).map((row, index) => (<tr key={index} className="border-t">
      ...
      {error && (<div className="mb-4 bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded">
          {error}
        </div>)}

      {success && (<div className="mb-4 bg-green-100 border border-green-400 text-green-700 px-4 py-2 rounded">
          {success}
        </div>)}

      <button onClick={handleImport} disabled={loading || importedData.length === 0} className="w-full bg-indigo-700 text-white font-semibold py-2 rounded hover:bg-indigo-800 transition disabled:opacity-50 disabled:cursor-not-allowed">
