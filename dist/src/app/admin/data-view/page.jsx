        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="space-y-4 max-h-96 overflow-y-auto">
              {substitutes.map(sub => (<div key={sub.id} className="border rounded p-4">
                  ...
                  <div className="mt-2">
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {sub.availability.Monday?.map((period, index) => (<span key={index} className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded">
                          {period}
                        </span>))}
                    </div>
                  </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="space-y-4 max-h-96 overflow-y-auto">
              {Object.entries(regularTeachers.reduce((acc, teacher) => {
            if (!acc[teacher.department])
                acc[teacher.department] = [];
            acc[teacher.department].push(teacher);
            return acc;
        }, {})).map(([dept, deptTeachers]) => (<div key={dept} className="border rounded p-4">
                  <h3 className="font-semibold text-gray-900">{dept}</h3>
                  <p className="text-sm text-gray-600">{deptTeachers.length} teachers</p>
                  <div className="mt-2">
                    {deptTeachers.slice(0, 3).map(teacher => (<div key={teacher.id} className="text-sm text-gray-700">
                        {teacher.name}
                      </div>))}
                    {deptTeachers.length > 3 && (<div className="text-sm text-gray-500">
