    ...
    return (<div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow">
          {activeTab === 'overview' && (<DatabaseOverview stats={stats}/>)}
          
          {activeTab === 'master-schedule' && (<MasterScheduleView masterSchedule={masterSchedule} periods={periods}/>)}
          
          {activeTab === 'substitutes' && (<SubstitutePoolView substitutes={substitutes} days={days} periods={periods}/>)}
          
          {activeTab === 'paraprofessionals' && (<ParaprofessionalView teachers={teachers} masterSchedule={masterSchedule} periods={periods}/>)}
          
          {activeTab === 'teachers' && (<TeachersView teachers={teachers} masterSchedule={masterSchedule} periods={periods}/>)}
        </div>
      </div>
    </div>);
}
function DatabaseOverview({ stats }) {
    if (!stats)
...
function MasterScheduleView({ masterSchedule, periods }) {
    // Group by teacher
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              {periods.map(period => (<th key={period} className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  {period}
                </th>))}
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-gray-200">
            {Object.values(teacherSchedules).map(({ teacher, schedule }) => (<tr key={teacher.id}>
                ...
                return (<td key={period} className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {periodData ? (<div className={`p-2 rounded ${periodData.isTeaching ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>
...
function SubstitutePoolView({ substitutes, days, periods }) {
    return (<div className="p-8">
      <h2 className="text-2xl font-bold text-gray-900 mb-6">Substitute Pool</h2>
      
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {substitutes.map(sub => (<div key={sub.id} className="bg-gray-50 p-6 rounded-lg">
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {sub.subjectSpecialties.map((specialty, index) => (<span key={index} className="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded">
                    {specialty}
                  </span>))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {days.map(day => (<div key={day} className="flex items-center">
                    <span className="w-20 text-sm font-medium text-gray-700">{day}:</span>
                    <div className="flex flex-wrap gap-1">
                      {sub.availability[day] ? (sub.availability[day].map(period => (<span key={period} className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded">
                            {period}
                          </span>))) : (<span className="text-gray-400 text-sm">Not available</span>)}
                    </div>
                  </div>))}
              </div>
            </div>
...
function ParaprofessionalView({ teachers, masterSchedule, periods }) {
    ...
    return (<div className="p-8">
      ...
      {paraprofessionals.length === 0 ? (<div className="text-center py-12">
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {paraSchedules.map(({ para, schedule }) => (<tr key={para.id}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                    {para.name}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
...
function TeachersView({ teachers, masterSchedule, periods }) {
    ...
    return (<div className="p-8">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {teacherSchedules.map(({ teacher, schedule }) => (<tr key={teacher.id}>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                  {teacher.name}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
